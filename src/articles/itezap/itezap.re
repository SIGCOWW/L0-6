...
この原稿執筆時、新キャラのシルエットのみ公開されています（入稿当日、キュートに辻野あかりちゃんが追加されました）。
...
年齢と身長の非線形回帰式の結果を@<img>{cinderella_age_height_polymodel}に、年齢とバストの非線形回帰式の結果を@<img>{cinderella_age_bust_polymodel}に示します。
...
そこで説明変数を増やした場合にどれくらい精度が上がるのかを調査します。
