...
でも最近は、「商品QR」@<fn>{yomitoru}という機能が導入され、これと使えばSDKやAPIが何か、手間はどうなるといったことを意識しなくても良いかもしれません。こっちの方が楽です。おわり。
//footnote[yomitoru][PayPayでいうところの「よみとる決済」です。]
...
//footnote[zettai][SquareやCoiney、PayPayのリバースエンジニアリングについては、当該サービスを使えなくなると困る上、加盟店情報交換センターに強制解約の情報が載ると非常に困るので絶対にやりません。]
...
画面タップのエミュレートには、iOSの機能として備わる「スイッチコントロール」を利用しました。
スイッチコントロールとは、



なお、Androidではここまで複雑にせずとも、adbのシェルから〜
またiOS端末でもSPIを〜勇気がなかった（Jailbreakは規約で禁止、規約に違反すると無理なので〜）
