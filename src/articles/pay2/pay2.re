...
申し込みすぎでは？
楽天ペイは交通系電子マネーを餌に楽天経済圏を広げるためだろうから審査が甘いだろう、と思ったらそんなことはありませんでした。
...
売り子でも簡単に各種決済を扱えることを目的@<fn>{toha}にしており、構成はRaspberry Piを中心としてiPadやバーコードリーダー、カスタマディスプレイが接続されています。
//footnote[toha][ただし、目的を達成しているとは言い難い状況です。]

ちなみにiPadのスタンドは@<img>{sigcoww_stand}の「Universal Tablet Station」、ブレンド・Sに出てきたスタンドと同じなため、SIGCOWWはスティーレだったんですね。
//subfig[Squareが提供する端末]{
//image[sigcoww_stand][SIGCOWWのiPadスタンド][scale=0.3]
//image[stille_stand][スティーレのiPadスタンド（ブレンド・S 8話より ©中山幸・芳文社／ブレンド・S製作委員会）][scale=0.65]
//}
...
=== Coiney
CoineyにもSDKがあるようです。
ある@<bou>{よう}ですというのは、余裕がなく今回のC95には実装が間に合わなかったためです。
このSDKは問い合わせにより仕様が開示されるとのことで詳細は不明ですが、概要を見る限り自作アプリにCoiney側で用意したViewを組み込むことで連携を行うように思えます。

Custom URL Schemeで連携してくれると嬉しいのですが、そういった記述は見つけられませんでした。
ただ、@<tt>{coiney://}のリンクを踏めば決済アプリが起動した@<fn>{reverse}ので、ランチャーアプリのようなものは作れるかもしれません。
//footnote[reverse][規約によりリバースエンジニアリングは禁止されているため、勘で当てましたがそのままでした。]
...
//footnote[zettai][SquareやCoineyのリバースエンジニアリングについては、当該サービスを使えなくなると困る上、加盟店情報交換センターに強制解約の情報が載ると非常に困るので絶対にやりません。]
...
そこでSIGCOWWでは、Adafruitのページ@<fn>{adafruit}を参考にBluetoothキーボードとして振る舞うデバイスをiPadのWebブラウザから操作し、最終的にiPad本体を操るという回りくどいことをしていました。
//footnote[adafruit][@<href>{https://learn.adafruit.com/ios-switch-control-using-ble/overview}]
...
//footnote[raspi-keyboard][@<href>{https://qiita.com/mt08/items/5f9dfc30707f94e3b8c8}]
...
iOS端末のタッチスクリーンコントローラはSPI接続らしい、というくらいしか分かりませんでしたが、リプレイ攻撃のように画面上のある位置をタップした際の信号を記録しておき、あとでその信号を送ればいい感じにできそうです。
Android端末では実現できているようで、サプライチェーン攻撃に利用されるかも知れないという指摘@<fn>{omer2018}や、スクフェスの物理ボタンを制作した例@<fn>{lovelive}がありました。
//footnote[omer2018][@<href>{https://iss.oy.ne.ro/Shattered}]
//footnote[lovelive][@<href>{https://www.nicovideo.jp/watch/sm26846522}]
...
それは、故意か否かはともかく、実際には支払われていないにも関わらず「支払った」と主張されることです。
...
@<img>{paypay-logo}は、テスト決済と返金を行った後、加盟店ロゴを変更した上で再びテスト決済と返金を行った際の利用者側アプリ画面です。
この仕様を利用して、イベントや時間帯ごとに加盟店ロゴを変更、もし決済完了画面に想定外のロゴが表示されていたら加盟店用のWebページから決済履歴を確認するという対策が取れそうです。
//image[paypay-logo][PayPayアプリの決済履歴画面（決済番号は加工）][scale=0.5]

本当は、Webhookで通知@<fn>{paypay-mail}、またはアプリか何かで通知してくれると決済完了の確認が簡単になりそうですが、そもそもそうした加盟店向けのアプリは提供されていません。
残念でした。
せめて、利用者用のアプリと何か連携できるか調べてみよう…としましたが、利用規約にリバースエンジニアリングの禁止が書かれていたためやめておきます。
たとえ利用者向けであっても規約違反は絶対にしません。
...
//image[acrylic_stand][SIGCOWWのiPadスタンド][scale=0.5]
