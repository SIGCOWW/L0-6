...
@<img>{paypay_logo}は、テスト決済と返金を行った後、加盟店ロゴを変更した上で再びテスト決済と返金を行った際の利用者側アプリ画面です。モノクロだと分かりにくいかも知れませんが、加盟店ロゴの色が変わっています。
この仕様を利用して、イベントや時間帯ごとに加盟店ロゴを変更、もし決済完了画面に想定外のロゴが表示されていたら加盟店用のWebページから決済履歴を確認するという対策が取れそうです。
//image[paypay_logo][PayPayアプリの決済履歴画面（決済番号は塗りつぶしています。）][scale=0.6]
...
「@<tt>{paypay://payments/加盟店ID}」で支払えると、PayPayのアプリを開かずにiOS標準のカメラだったり他のQRコードリーダーからも遷移できたりして利便性が向上する気もします@<fn>{alipay}。
...
今後は、PayPayで「みせる決済」を受けられるようにしたり、QUICPayやiD、クレジットカードの非接触決済を入れたりしたいですね。
ただ、実現可能性は低いです。
